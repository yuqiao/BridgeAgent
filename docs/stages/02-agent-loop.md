...
状态：代码、本地确定性验收与 B11 真实模型闭环均已完成；最后复验 1 passed（15.39 秒），历史限流/超时记录保留如下。用户已授权阶段 2 与本地 `.env` 验证，并确认下列 TDD 测试边界。使用方式见 [Agent 指南](../guides/agent.md)。
...
| B11 | `.env` 真实端点至少完成一次工具闭环和同会话后续回答 | 已通过：原 .env 完成 add → 4812 → 最终回答 → 同会话追问且未重复调用工具；1 passed，15.39 秒。默认 CI 不调用外部模型 |
...
保留用户配置，不自动更换模型、服务地址或密钥。当时 B11 未通过，端点恢复或用户调整配置后可重跑 `uv run --locked pytest tests/test_agent_live.py --run-live -v --tb=short`。本地 HTTP 协议测试结果独立记录，不替代真实服务验收。
...
### B11 关闭验收（2026-09-25，stage7 之后）

用户要求继续后，使用原 `.env` 和既有 600 秒超时，不改动测试、应用代码、模型、服务地址或凭据，执行：

```bash
UV_DEFAULT_INDEX=https://pypi.org/simple uv run --locked pytest tests/test_agent_live.py --run-live -q
# 1 passed in 15.39s
```

全部既有断言通过：真实执行 add(1847, 2965) 返回 4812；模型最终回答引用结果；同一会话追问返回 4812，且本轮没有再次调用工具。B11 现在通过，阶段 2 验收关闭。

最初直接运行 uv 命令遇到本机镜像与官方 PyPI 锁文件不一致；仅将本次命令的 UV_DEFAULT_INDEX 指向锁定源后运行，未更新锁文件或依赖。这与先前 HTTP 429 是两个独立问题。

本次成功表明当前端点可完成验收，不足以确定先前 429 的供应商内部原因，也不保证持续可用性。没有为一次外部恢复引入重试策略或修改实现。此次仅更新验收文档，保留历史失败与所有已发布 tag。
