...
状态：阶段 0–7 的代码和本地验收已完成。具备 Agent 循环、文件与 Skill 工具、SQLite、审批与命令、外部插件及动态宿主；真实仓库问答已通过，加法/多轮验收仍限流。逐项证据见 [路线图](roadmap.md)。
...
首个入口为 CLI；首期插件是可信、同进程的 Python 代码，启动时显式装配。阶段 6 已提供第三方包发现；阶段 7 已提供 Cordis 风格动态依赖、上下文作用域、事件与热重载。不可信插件隔离另行评估。

首个真实任务为读取指定仓库、回答代码问题并给出文件位置。模型插件接入 OpenAI 或兼容服务，模型名称和服务地址可配置。阶段 5 已提供审批约束下的文件修改与命名命令，独立于只读验收。

阶段 3.5 通过 Skill 插件提供发现、按需加载 `SKILL.md` 与参考资料、上下文接入能力。Skill 是任务指导与资源，插件是承载该机制的程序实现；Agent 依指导调用已有工具。先支持只读流程，阶段 5 已扩展文件修改与脚本执行，不依赖阶段 7 的动态插件机制。

## 模块分层

下列目录从阶段 1 建立，后续阶段持续遵守同一依赖方向。
...
阶段 7 扩展为完整动态生命周期，按参考基线建立逐项行为对齐与测试矩阵。阶段 1 的静态依赖验证、阶段 7 的运行中依赖变化分别定义，不能用首期遇到缺失依赖就失败的规则代替动态停用与重激活。DynamicHost 与静态 PluginHost 分开：动态服务缺失进入 pending；变更通过租约排空调用，失败恢复旧依赖链。CLI 显式 --dynamic 启用 v2 entry 树，动态运行时适配器每轮解析当前服务；application 不知道热重载。具体接口与边界见 [动态插件指南](guides/dynamic-plugins.md)。
...
阶段 4 已使用 SQLite 保存 checkpoint，仅续接已完成且兼容的轮次；中断会话要求新 ID。阶段 5 对修改/命令逐次审批，单宿主请求去重，不跨进程自动重放副作用；checkpoint 不提供外部动作的 exactly-once 保证。
