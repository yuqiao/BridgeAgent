...
状态：设计基线已定稿，应用配置使用 YAML 显式装配。阶段 1 已实现分层、最小插件宿主与演示；本文中的 Agent、模型、工具和会话能力按后续阶段推进。
...
模型名称与服务地址可配置，密钥字段只引用环境变量名称；这些字段在阶段 2 接入。阶段 1 已验证 `version` / `plugins` 格式，采用 PyYAML 与 Pydantic，不提供通用环境插值。Python 包元数据与开发工具配置仍使用标准的 `pyproject.toml`，与应用装配配置分开。
