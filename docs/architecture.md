...
状态：设计基线已定稿，应用配置使用 YAML 显式装配。本文描述目标架构，功能尚未实现。
...
首个入口为 CLI；首期插件是可信、同进程的 Python 代码，启动时显式装配。第三方包发现、热重载和不可信插件隔离后置。
...
模型名称与服务地址可配置，密钥字段只引用环境变量名称。具体 YAML 字段和解析库在阶段 1 细化并验证。Python 包元数据与开发工具配置仍使用标准的 `pyproject.toml`，与应用装配配置分开。
